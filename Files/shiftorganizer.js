...
function buildPage(password){	
	...
	if(flag == 0)
	{
		...
		if(!checkValid(password,locForS)){
			swal({   title: ".שם משתמש וסיסמא אינם תקינים",     timer: 1000,   showConfirmButton: false });
				return;
		}
...
function cleanSidur(){
	swal({   title: "האם אתה בטוח שברצונך למחוק את הסידור", 
	text: ".לא תוכל לשחזר מידע זה",
	type: "warning",
...
function shiftInserted(){
	...
	swal({   title: ".הכנסת משמרות בוצעה בהצלחה",   imageUrl: "Files/sweetalert-master/like.gif" });
...
function moveEmployeer(itemPass) {
	...
	swal({title: "האם אתה בטוח שברצונך למחוק עובד זה?",
		  type: "warning",
		 showCancelButton: true,
		confirmButtonColor: "#DD6B55", 
		confirmButtonText: "Yes",  
...
function insertNewEmployee(){
	if($("#pName").val() == ""){
		swal("שם פרטי לא הוכנס");
		return;
	}
	if($("#lName").val() == ""){
		swal("שם משפחה לא הוכנס");
		return;
	}
	if($("#phone").val() == ""){
		swal("טלפון לא הוכנס");
	...
	if($("#password").val() == ""){
		swal("סיסמא לא הוכנסה");
		return;
	}
	if($("#password").val() == "1111"){
		swal("הוכנסה סיסמת מנהל- אנא הכנס סיסמה אחרת");
		$("#password").val("");
		return;
	}
	...
	if(isNaN(tmp)===true){
		swal("אנא הכנס ססמא בעלת מספרים בלבד");
		$("#password").val("");
		return;
	}
